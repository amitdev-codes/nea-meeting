...
class StarterCategoryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('checkbox', fn ($row) => $this->renderCheckbox('starter-categories[]', $row->id))
            ->addColumn('status', fn ($row) => $this->getStatusBadge($row->status))
            ->addColumn('action', $this->addActionColumn(
                'modal',
                $this->getRoutes(),
                $this->getPermissions('starter-categories')
            ))
            ->rawColumns(['checkbox', 'action','status']);
    }

    public function query(StarterCategory $model): QueryBuilder
    {
    ...
    {
        $routeName = route("admin.starter-categories.import");
        return $this->builder()
            ->setTableId('starter-categories-table')
            ...
            ->buttons(
                array_merge(
                    $this->dtActionModalButtons('starter-categories','StarterCategory'),
                    $this->importButton($routeName) // Pass the route name here
                )
            )
            ->parameters([
                'initComplete' => 'function() {
                    ' . $this->initBulkDeleteScript('starter-categories[]') . '
    ...
    {
        return 'StarterCategory_' . date('YmdHis');
    }

    protected function getRoutes(): array
    {
        return [
            'create' => 'admin.starter-categories.create',
            'view' => 'admin.starter-categories.show',
            'edit' => 'admin.starter-categories.edit',
            'delete' => 'admin.starter-categories.destroy',
        ];
    }
